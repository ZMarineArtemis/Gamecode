...
label dialogue8:
    ...
    python:
        import os.path
        import os
        savePath = config.basedir + "/resource/"
        if not os.path.exists(savePath):
            os.makedirs(savePath)
        complete_dir = os.path.join(savePath, "examplewrite" + ".txt")
        file = open(complete_dir, "r+")
        threshold = True
        while threshold:
            input_txt = renpy.input('Press Enter to turn the page, write END ONLY in a page to leave the file: ', length = 210)
            if input_txt != 'END':
                file.write("{}\n".format(input_txt))
            else:
                threshold = False
    ...
    u "You text is saved under the 'resource' file below the game root"
    u "Thanks for your playing"
